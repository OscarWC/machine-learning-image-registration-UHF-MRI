...
for i in range(1, 101):
    with open(f"2D skull mesh/00{i}_augments.txt") as f:
        lines = f.readlines()
        line1 = lines[0]
    ...
    with open(f"2D skull mesh/reg_00{i}.txt") as g:
        lines = g.readlines()
        line1 = lines[0]
    ...
    gtruths.append([r, x, y])
    # registration parameters
    regpar.append([rr, xr, yr])
...
for n in range(3):
    ...
    text_file = open(f"2D skull mesh/analysis/statistics.txt", "a")
...
for o in range(3):
    ...
    plt.savefig(f'2D skull mesh/analysis/graph_{o}')
    plt.close()

# form composite image to display all graphs in one
images = [Image.open(f"2D skull mesh/analysis/graph_{o}.png") for o in o_map]
...
new_image.save(f"2D skull mesh/analysis/graphs.png")